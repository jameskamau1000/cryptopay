...
class BscAdapter extends BaseChainAdapter implements ChainAdapterInterface
{
    ...
    {
        $this->guardEnabled('bsc');
        $rpcUrl = (string) config('chains.bsc.rpc_url');
        $contract = strtolower((string) config('chains.bsc.usdt_contract'));

        if (!$rpcUrl || !$contract) {
            throw new RuntimeException('BSC RPC URL and USDT contract must be configured');
        }

        $normalizedAddress = $this->normalizeHexAddress($address);
        $latestHex = $this->rpc('eth_blockNumber', []);
        $latestBlock = hexdec((string) $latestHex);
        $window = (int) config('chains.bsc.scan_window_blocks', 5000);
        ...
        $transfers = [];
        foreach (($logs ?: []) as $log) {
            $amount = $this->hexTokenAmountToFloat((string) ($log['data'] ?? '0x0'), 18);
    ...
    public function sendAsset(array $transfer): array
    {
        ...
        if (empty($transfer['from_address']) || empty($transfer['to_address'])) {
            throw new RuntimeException('Invalid BSC transfer payload');
        }
        ...
        } elseif ((bool) config('chains.bsc.node_managed_signing', false)) {
        ...
        if (!$txHash || !is_string($txHash)) {
            throw new RuntimeException('BSC RPC returned empty transaction hash');
        }

        return [
            'tx_hash' => $txHash,
            'fee' => 0.0,
            'payload' => ['chain' => 'bsc', 'broadcast' => $broadcastMethod],
        ];
    }
    ...
    private function sendTokenFromManagedAccount(array $transfer): string
    {
        $contract = strtolower((string) config('chains.bsc.usdt_contract'));
        if (!$contract) {
            throw new RuntimeException('BSC USDT contract must be configured');
        }

        $from = $this->normalizeHexAddress((string) $transfer['from_address']);
        $to = $this->normalizeHexAddress((string) $transfer['to_address']);
        $data = $this->encodeErc20TransferData($to, (string) ($transfer['amount_str'] ?? $transfer['amount'] ?? '0'), 18);
        ...
        $gas = (int) config('chains.bsc.token_transfer_gas', 120000);
        if ($gas > 0) {
        ...
        if (!is_string($txHash) || $txHash === '') {
            throw new RuntimeException('BSC node-managed send failed');
        }
        return $txHash;
    }
    ...
    {
        $rpcUrl = (string) config('chains.bsc.rpc_url');
        $rpcKey = (string) config('chains.bsc.rpc_key');
        if (!$rpcUrl) {
            throw new RuntimeException('BSC RPC URL is not configured');
        }
        ...
        if (($json['error'] ?? null) !== null) {
            $message = $json['error']['message'] ?? 'BSC RPC error';
    ...
    private function normalizeHexAddress(string $address): string
    {
        ...
        if (!preg_match('/^0x[a-f0-9]{40}$/', $address)) {
            throw new RuntimeException('Invalid BSC address format');
        }
    ...
    private function hexTokenAmountToFloat(string $hexValue, int $decimals): float
    {
        ...
        // Fits in 64-bit integer for practical payment amounts.
        ...
        // Fallback without BCMath/GMP: approximate for small transfers.
