...
class EthereumAdapter extends BaseChainAdapter implements ChainAdapterInterface
{
    ...
    public function generateAddress(array $context = []): array
    {
        ...
        if (config('chains.signer.enabled')) {
            $provided = $this->chainSignerService->provisionAddress('eth', (string) ($context['asset'] ?? config('chains.default_asset', 'USDT')), $context);
            return [
        ...
        if ((bool) config('chains.eth.node_managed_signing', false)) {
            $newAddress = $this->rpc('personal_newAccount', [(string) config('chains.eth.account_password', '')]);
            if (is_string($newAddress) && $newAddress !== '') {
                return ['address' => $this->normalizeHexAddress($newAddress), 'memo' => null];
            }
            throw new RuntimeException('ETH node-managed account creation failed');
        }

        throw new RuntimeException('ETH self-custody address provisioning requires unified signer or node-managed account API');
    }
