...
Route::middleware(['admin', 'admin.ip.allowlist'])->group(function () {
    ...
    Route::controller('OperationsController')->prefix('operations')->name('operations.')->group(function () {
        Route::get('invoices', 'invoices')->name('invoices');
        Route::post('invoices/{id}/status', 'invoiceStatus')->name('invoices.status');
        Route::get('payouts', 'payouts')->name('payouts');
        Route::post('payouts/{id}/status', 'payoutStatus')->name('payouts.status');
        Route::post('payouts/freeze-toggle', 'payoutFreezeToggle')->name('payouts.freeze.toggle');
        Route::get('api-keys', 'apiKeys')->name('api.keys');
        Route::post('api-keys/{id}/status', 'keyStatus')->name('api.keys.status');
        Route::post('api-keys/{id}/regenerate-secret', 'keyRegenerateSecret')->name('api.keys.regenerate');
        Route::get('webhook-endpoints', 'webhookEndpoints')->name('webhook.endpoints');
        Route::post('webhook-endpoints/{id}/status', 'endpointStatus')->name('webhook.endpoints.status');
        Route::post('webhook-endpoints/{id}/rotate-secret', 'endpointRotateSecret')->name('webhook.endpoints.rotate');
        Route::get('webhooks', 'webhookDeliveries')->name('webhooks');
        Route::post('webhooks/{id}/retry', 'retryDelivery')->name('webhooks.retry');
        Route::get('api-logs', 'apiLogs')->name('api.logs');
        Route::get('risk-cases', 'riskCases')->name('risk.cases');
        ...
        Route::get('deposit-addresses', 'depositAddresses')->name('deposit.addresses');
        Route::get('onchain-deposits', 'onchainDeposits')->name('onchain.deposits');
        Route::get('onchain-payouts', 'onchainPayouts')->name('onchain.payouts');
    });
